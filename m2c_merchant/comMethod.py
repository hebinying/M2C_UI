...
goodsdata={
    'logisticsType.for':0,
    'goodsName.for':"非手动新增实物商品"+str(datetime.date.today().strftime("%Y-%m-%d")),
    'goodsSubTitle.for':"副标题副标题副标题",
    ...
    'mainImg.class':'./config/商品主图.jpg',#商品主图class
    'videoContainer.id':'./config/商品视频.mp4',#商品视频id
    'editor-container.id':'',#图文详情id div#id iframe#ueditor_0
    'setupmethod':'',#通过查找：设置上架 wording找其父亲div的兄弟，可不用
    'commit.class':'',#class poi3>button
    'goodsPostageId':'',#运费模板id,只有实物商品存在
    'addressName':'',#不配送区域，方式如何？
...
if __name__=="__main__":
    print goodsdata.keys()
    print goodsdata.values()
    print goodsdata.has_key("graySpan")
